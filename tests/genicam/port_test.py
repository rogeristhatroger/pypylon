...
class PortTestSuite(GenicamTestCase):
    def test_PortAccess(self):
        ...
        Port = CTestPort()
        MyRegister = 42
        Port.CreateEntry(0x00ff, "uint32_t", MyRegister, genicam.RW, genicam.LittleEndian)
        ...
        self.assertEqual(MyRegister, cast_buffer("uint32_t", genicam.LittleEndian, Buffer))

        # write a new value
        MyNewRegister = 99
        port.Write(0x00ff, cast_data("uint32_t", genicam.LittleEndian, MyNewRegister))
        Buffer = port.Read(0x00ff, sizeof("uint32_t"))
        self.assertEqual(MyNewRegister, cast_buffer("uint32_t", genicam.LittleEndian, Buffer))
    ...
    def test_TestPort(self):
        ...
        Port = CTestPort()
        MyRegister = 42
        Port.CreateEntry(0x0000, "uint32_t", 42, genicam.RW, genicam.LittleEndian)
        Port.CreateEntry(0x000f, "uint32_t", MyRegister, genicam.RW, genicam.LittleEndian)
        MyRegisterLittleEndian = MyRegister
        Port.CreateEntry(0x00ff, "uint32_t", MyRegisterLittleEndian, genicam.RW, genicam.LittleEndian)
        ...
        MyRegister = 42
        Port.CreateEntry(0x00ff, "uint32_t", MyRegister, genicam.RO, genicam.LittleEndian)
        ...
        # Create an WO entry
        MyRegister = 43
        Port.CreateEntry(0x000f, "uint32_t", MyRegister, genicam.WO, genicam.LittleEndian)
        ...
        MyRegister = 42
        Port.Write(0x00ff, cast_data("uint32_t", genicam.LittleEndian, MyRegister))
        ...
        with self.assertRaises(genicam.GenericException):
            Port.Write(0x0000, cast_data("uint32_t", genicam.LittleEndian, MyRegister))
        ...
        # make the entry RO
        MyRegister = 0
        Port.CreateEntry(0x000f, "uint32_t", MyRegister, genicam.RO, genicam.LittleEndian)

        # try to write to a RO address
        with self.assertRaises(genicam.GenericException):
            Port.Write(0x000f, cast_data("uint32_t", genicam.LittleEndian, MyRegister))
    ...
    def test_PortRef(self):
        ...
        port.Write(0x00ff, cast_data("uint32_t", genicam.LittleEndian, WriteBuffer))
        ReadBuffer = port.Read(0x00ff, sizeof("uint32_t"))
        self.assertTrue(cast_buffer("uint32_t", genicam.LittleEndian, ReadBuffer) == WriteBuffer)
        ...
        testReg.SetValue(42)
        ReadBuffer = port.Read(0x00ff, sizeof("uint32_t"))
        self.assertTrue(cast_buffer("uint32_t", genicam.LittleEndian, ReadBuffer) == 42)
    ...
    def test_Chunk(self):
        ...
        self.assertEqual(genicam.NA, Port.GetAccessMode())
        self.assertTrue(Port.GetChunkID() == "12")

        self.assertTrue(Port.GetSwapEndianess() == genicam.No)
        ...
        self.assertEqual(genicam.NI, Port2.GetAccessMode())
        self.assertTrue(Port2.GetChunkID() == "")

        self.assertTrue(Port2.GetSwapEndianess() == genicam.No)
    ...
    def test_Chunk2(self):
        ...
        TestValue = "30"
        MyPort.CreateEntries(0x00A0, 0x00E0, "uint32_t", TestValue, genicam.RW, genicam.LittleEndian)

        # connect the node map to the port
        Camera3._Connect(MyPort, "MyPort")

        # Manits #95
        self.assertEqual(genicam.NA, Port3.GetAccessMode())
        self.assertTrue(Port3.GetChunkID() == "30")
        PortConstr3 = Camera3._GetNode("BogusPort")
        self.assertTrue(PortConstr3.GetSwapEndianess() == genicam.No)
