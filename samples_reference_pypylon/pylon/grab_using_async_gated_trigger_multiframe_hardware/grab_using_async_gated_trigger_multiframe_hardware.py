...
The gate signal is expected on a physical GPIO line (FrontGPI3 by
default). For a software-simulated variant that does not require
external trigger hardware see grab_using_async_gated_trigger_multiframe.

This sample requires a CXP line scan camera connected through a Basler
CXP-12 frame grabber and an external trigger signal connected to
FrontGPI3.
"""
import sys
import time
    ...
    while camera.IsGrabbing():
        with camera.RetrieveResult(
            ...
            if grab_result.GrabSucceeded():
                ...
                current_height = grab_result.PayloadSize // grab_result.Width
                if is_sequence_end:
...
try:
    ...
    with pylon.InstantCamera() as camera:
        ...
        # Use a hardware GPIO line as the gate signal source.
        transport_layer_nodemap.ImageTriggerInputSource.Value = "TriggerInSourceFrontGPI3"

        # --- Configure the hardware trigger line ---
        camera.LineSelector.Value = "Line2"
        camera.LineInverter.Value = False
        camera.LineDebouncerTimeAbs.Value = 0.0
        print("Hardware trigger line configured (Line2, FrontGPI3)")
